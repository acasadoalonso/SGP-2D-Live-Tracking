...
/*
$ch = curl_init($appurl."/node/agl.php");
...
if($FLAG=='NEARBY'){
	...
	$resultBase = $connAPRSLOG->query($sqlString);
	if ($resultBase->num_rows > 0) {
		...
		$resultBase2 = $connAPRSLOG->query($sqlString);
...
if($FLAG==''){
	...
	if ($activeFlarm!=""&&$lat!=0&&$lon!=0){
		...
		$resultWP = $connAPRSLOG->query($sqlString);
	...
	$max=0;
	$resultCover = $connAPRSLOG->query($sqlString);
