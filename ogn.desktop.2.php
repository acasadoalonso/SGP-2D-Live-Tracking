...
<script src="https://maps.googleapis.com/maps/api/js?libraries=geometry&key=AIzaSyCKOPCAqnZW-OZvw3hzOjcKTldrZZN9wLo"></script>
...
<div class="subMenu" type="transmiters" style="right:10px; border-right:1px solid #D6D4D1;" id="transmitersCount">TX</div>
<div class="subMenu" type="receivers" style="right:50px; border-left:1px solid #D6D4D1;" id="receiversCount">RX</div>
<div class="subMenu" type="waypoints" style="right:90px; width:40px!important;" id="waypointsCount">WPS</div>
<div class="subMenu" type="terrain" style="right:130px; width:30px!important;border-left:1px solid #D6D4D1;" id="terrain"><img src="/node/img/terrain.png" width="20" height="20"></div>
<div class="subMenu" type="visible" style="right:160px; width:30px!important;border-left:1px solid #D6D4D1;" id="visible"><img src="/node/img/visible.all.png" width="20" height="20"></div>
<div class="subMenu" type="pallete" style="right:190px; width:30px!important;border-left:1px solid #D6D4D1;" id="pallete"><img src="/node/img/pallete.png" width="20" height="20"></div>
<style>
...
<div class="pallete">
<strong>Pallete Options</strong><br>
<em>&nbsp;&nbsp;Gliders</em><br>
...
var tx={}, rx={};
var busy=false;
var socket = io.connect('http://ogn.planeadores.cl:81');
...
$('#visible').off('click').on('click', function(){
	busy=true
	$.cookie("visible")=="all" ? $.cookie("visible","online") : $.cookie("visible", "all");
	$("#visible").children().attr("src", "/node/img/visible." + $.cookie("visible") + ".png");
...
$(document).off('click', '.subMenu').on('click', '.subMenu', function(){
	...
	if($(this).attr('type')=='terrain'){
		if($.cookie("mapType")=="hybrid"){
			$.cookie("mapType", "roadmap")
		}else if($.cookie("mapType")=="roadmap"){
			$.cookie("mapType", "satellite")
		}else if($.cookie("mapType")=="satellite"){
			$.cookie("mapType", "terrain")
		}else if($.cookie("mapType")=="terrain"){
			$.cookie("mapType", "hybrid")
		}
	...
	$.ajax({
		...
		success: function(json){ 
			...
			for(i=0;i<json.waypoints.length;i++){
				...
				var html='<img src="/node/img/antenna1.png" width="20" height="20" style="float:right;"><strong>' + data.waypoint + ' ' + data.waypointCountry + '</strong><br>Lat: ' + data.waypointLat + ' Lon: ' + data.waypointLon + '<br>'
					
				$node='<div class="waypoint" waypoint="' + data.waypoint + '" id="waypoint' + data.idWaypoint + '" lat="' + data.waypointLat + '" lon="' + data.waypointLon + '">' +
				'<div class="waypointsIcon" id="waypoints' + data.idWaypoint + '"></div>'+
...
$('#aprs').off('click').on('click', function(){
	...
	try{
		infowindow.open(map, rx[$(this).text()].marker);
		map.panTo(new google.maps.LatLng(rx[$(this).text()].lat, rx[$(this).text()].lon))
	}catch(e){}
